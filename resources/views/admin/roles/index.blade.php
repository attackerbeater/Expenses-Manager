                        ...
                        @foreach ($roles as $role)
                            <tr data-entry-id="{{ $role->id }}">
                                ...
                                <td>
                                    ...
                                    @if($identity == 1)
                                      @can('role_view')
                                      <a href="{{ $role->id == 1 ? 'javascript:void(0)' : route('admin.roles.show',[$role->id]) }}" class="btn btn-xs btn-primary">@lang('quickadmin.qa_view')</a>
                                      @endcan

                                      @can('role_edit')
                                      <a href="{{ $role->id == 1 ? 'javascript:void(0)' : route('admin.roles.edit',[$role->id]) }}" class="btn btn-xs btn-info">@lang('quickadmin.qa_edit')</a>
                                      @endcan
                                      @if($role->id == 1)
